    ...
    (async () => {
      try {
        ...
        if (res.ok && data?.policy) {
          setTitle(data.policy.title || 'Cancellation Policy');
    ...
    <div style={{ maxWidth: 820, margin: '0 auto', padding: '24px' }}>
      ...
      {sections.map((sec, idx) => (
        <div key={idx} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 14, marginBottom: 12, background: '#f8fafc' }}>
          ...
          <input
            style={inputStyle}
            placeholder="Heading (e.g. 1. Cancellation timeframe)"
            value={sec.heading}
      ...
      <label style={{ fontWeight: 700, fontSize: '0.85rem', display: 'block', marginBottom: 4 }}>Footer Agreement Note</label>
...
export default AdminCancellationPolicy;