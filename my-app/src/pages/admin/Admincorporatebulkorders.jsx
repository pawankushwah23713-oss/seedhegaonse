...
const AdminCorporateBulkOrders = () => {
  const [title, setTitle] = useState('Corporate & Bulk Orders');
    ...
    (async () => {
      try {
        ...
        if (res.ok && data?.policy) {
          setTitle(data.policy.title || 'Corporate & Bulk Orders');
          ...
          setFooterNote(data.policy.footerNote || '');
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE}/corporate-bulk-orders`, {
        ...
        body: JSON.stringify({ title, introText, sections: cleanSections, footerNote })
      });
      const data = await res.json();
    ...
    <div style={{ maxWidth: 820, margin: '0 auto', padding: '24px' }}>
      ...
      {sections.map((sec, idx) => (
        <div key={idx} style={{ border: '1px solid #e2e8f0', borderRadius: 8, padding: 14, marginBottom: 12, background: '#f8fafc' }}>
          ...
          <input
            style={inputStyle}
            placeholder="Heading (e.g. Advance booking & availability)"
            value={sec.heading}
      ...
      <label style={{ fontWeight: 700, fontSize: '0.85rem', display: 'block', marginBottom: 4 }}>Footer Note (shown in italics)</label>
      <textarea style={{ ...inputStyle, minHeight: 60 }} value={footerNote} onChange={(e) => setFooterNote(e.target.value)} />
...
export default AdminCorporateBulkOrders;