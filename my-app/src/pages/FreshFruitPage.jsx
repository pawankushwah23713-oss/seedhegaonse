...
const FALLBACK_CAKE_IMG = 'https://images.unsplash.com/photo-1565958011703-44f9829ba187?q=80&w=600&auto=format&fit=crop';
...
const FreshFruitPage = ({ addToCart, addedToast }) => {
  ...
  // 🟢 DIRECT BACKEND FETCH: Sirf 'fruit' category fetch hogi
  useEffect(() => {
    const fetchFruitCakes = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_BASE}/cakes?category=fruit`);
        const data = await res.json();
        if (res.ok && Array.isArray(data)) {
          setProducts(data.filter(item => item.category?.toLowerCase().includes('fruit')));
        }
      } catch (err) {
        console.error("Fruit cakes fetch error:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="ck-homepage-container">
              ...
              <div className="ck-modal-info-col">
                <div>
                  <div className="ck-modal-tags-row">
                    ...
                    <span className="ck-badge-category">🍓 FRUIT</span>
                  </div>
                  ...
                  <p className="ck-modal-desc">
                    {selectedProduct.description || 'Light vanilla sponge layered with freshly whipped cream and hand-cut seasonal fruits. Free birthday candle and knife included.'}
                  </p>
      ...
      <div style={{ background: '#064e3b', color: '#fff', padding: '35px 20px', textAlign: 'center' }}>
        <h1 style={{ fontSize: '28px', margin: '0 0 8px', color: '#6ee7b7' }}>🍓 Seasonal Fresh Fruit Cakes</h1>
        <p style={{ margin: 0, color: '#a7f3d0', fontSize: '14px' }}>Light Fresh Cream Loaded With Strawberries, Kiwi & Seasonal Fruits</p>
      </div>

      <div className="ck-container">
        {loading ? (
          <div style={{ textAlign: 'center', padding: '60px', color: '#64748b' }}>🍓 Loading Fruit Cakes...</div>
        ) : products.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '50px', background: '#fff', borderRadius: '12px' }}>
            <h3>No Fruit Cakes Available Right Now</h3>
          </div>
        ) : (
                ...
                >
                  ...
                  <div className="ck-card-top-bar" style={{ position: 'relative', zIndex: 6 }}>
                    {out ? (
                      ...
                      <span className="ck-badge-category-mini">🍓 FRUIT</span>
                    )}
                    <button
                      type="button"
